...
(function loadJSIL (config) {
  ...
  Environment_Browser.prototype.loadScript = function (uri) {
    ...
    document.write(
      "<script type=\"text/javascript\" src=\"" + uri + "?20223310728\" onerror=\"$scriptLoadFailed(" +
      this.scriptIndex +
      ")\"></script>"
    );
  };
  ...
  // for webworker environment, call "onLoad" when all is loaded
  if (config.environment == 'webworker')
    onLoad();
